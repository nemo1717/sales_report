...
router.get('/profile',  function(req, res, next) {
  db.query('SELECT  city_name, state_name, party_name, price, startDate, startTime, endDate, endTime, image, address, full_name  FROM register natural join party where userid = id', function (err, rs) {
    if(!err){
    res.render('profile', {party: rs, moment:moment});
    }
    else{
...
router.get('/search',function(req,res){
  ...
  db.query('SELECT  city_name, state_name, party_name, price, image, address, full_name  FROM register natural join party where userid = id and city_name LIKE "%'+req.query.search+'%" or state_name LIKE "%'+req.query.search+'%" or party_name LIKE "%'+req.query.search+'%" or address LIKE "%'+req.query.search+'%" or full_name LIKE "%'+req.query.search+'%" or price LIKE "%'+req.query.search+'%"' ,function(err, rows, fields) {
    if (err) throw err;
    res.render('click', {party: rows});
